<template>
    <div class="auth-content">
        <div class="auth-modal">
            <img src="@/assets/shoplist.png" width="200" alt="logo" />
            <hr>
            <div class="auth-title">{{ showSignup ? 'Cadastro' : 'Login' }}</div>

            <input v-model="user.username" type="text" placeholder="Nome">
            <input v-model="user.email" type="text" placeholder="E-mail">
            <input v-model="user.password" type="password" placeholder="Senha">
            <input v-if="showSignup" v-model="user.password_confirmation"
...
export default {
    ...
    methods: {
        signin() {
            axios.post(`${baseApiUrl}/api/v1/authenticate`, this.user)
                .then(res => {                    
        ...
        signup() {
            ...
            axios.post(`${baseApiUrl}/api/v1/users`, params)
                .then(() => {
